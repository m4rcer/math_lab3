...
public static class Cubic
{
    ...
    public static MyPoint[] InterpolateXY(MyPoint[] points, int count)
    {
        ...
        double[] xsOut = Interpolate(inputDistances, points.Select(p => p.X).ToArray(), evenDistances);
        double[] ysOut = Interpolate(inputDistances, points.Select(p => p.Y).ToArray(), evenDistances);
        return xsOut.Select((x, index) => new MyPoint(x, ysOut[index])).ToArray();
    }

    public static double[] Interpolate(double[] xOrig, double[] yOrig, double[] xInterp)
    {
